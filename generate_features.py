...
def recomputeTimestamp(data,start_time):
    co = data.drop_duplicates(ignore_index=False)
    co.timestamp = pd.to_datetime(co['timestamp'])
    co = co.reset_index()
    co.timestamp = co.timestamp - start_time
    co.timestamp = pd.Series([val.total_seconds() for val in co.timestamp])
    co = co.reset_index()
    return co

def feature_level_fusion(log_file,vad_file,speech_file,start_time,end_time,window,group,output_file_name,mapping_file,prefix):
    """
    ...
    # vad, speech, log files after sync
    sync_log_mask = (log['timestamp'] > start_time) & (log['timestamp'] <= end_time)
    sync_vad_mask = (vad['timestamp'] > start_time) & (vad['timestamp'] <= end_time)
    sync_speech_mask = (speech['timestamp'] > start_time) & (speech['timestamp'] <= end_time)

    sync_log = recomputeTimestamp(log[sync_log_mask],start_time)
    sync_vad = recomputeTimestamp(vad[sync_vad_mask],start_time)
    sync_speech = recomputeTimestamp(speech[sync_speech_mask],start_time)




    sync_log_name = prefix + '_group_' + str(group) + '_sync_log_features.csv'
    sync_vad_name = prefix + '_group_' + str(group) + '_sync_vad_features.csv'
    sync_speech_name = prefix + '_group_' + str(group) + '_sync_speech_features.csv'

    sync_log.to_csv(sync_log_name,index=False)
    sync_vad.to_csv(sync_vad_name,index=False)
    sync_speech.to_csv(sync_speech_name,index=False)
    ...
    while(frame_start <= end_time):
        ...
        df_vad = df_vad.drop_duplicates(ignore_index=True)
        df_speech = df_speech.drop_duplicates(ignore_index=True)

        sequence = df_vad['user'].to_list()
...
def generateFeatures(video_dir,log_file,vad_file,speech_file,window,output_file_prefix,mapping_file):
    ...
    # extract timestamp from video file name
    for key in dirlist.keys():
        ...
        dddd = feature_level_fusion(log_file,vad_file,speech_file,start_time,end_time,window,key,output_file_name,mapping_file,output_file_prefix)
