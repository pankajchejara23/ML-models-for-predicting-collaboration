...
def extractMediaPipeFeatures(video_file,show_flag=False):
...
if len(sys.argv) < 3:
    ...
    print("Use this format: \n python extractFaceLandmarks.py <source_directory> <target_directory>")
    ...
    for f in os.listdir(directory):
        ...
        if len(f_split) == 6 and f_split[5] == 'VIDEO' and f.split(".")[1] == 'mov':
            print('    Processing file:',f)
            feature_file_ele = f_split[:3] + ['face','pose','hand']
            feature_file_name = '_'.join(feature_file_ele)
            feature_file_name = target + '/' + feature_file_name + '.csv'

            df = extractMediaPipeFeatures(full_file_name)
            df.to_csv(feature_file_name,index=False)
            print('    Features saved in  ',feature_file_name)
